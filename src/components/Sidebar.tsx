...
import { 
  FiHome, FiUpload, FiList, FiPieChart, 
...
export default function Sidebar() {
  ...
  return (
    <aside className="w-72 border-r border-slate-200 min-h-screen bg-white flex flex-col p-6 relative">
      ...
      {/* 2. Barra de Busca na Sidebar (Dá um toque bem moderno) */}
      ...
      <nav className="flex-1 flex flex-col gap-1.5">
        <p className="px-3 text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 mb-2">Menu Principal</p>
      ...
      <div className="pt-6 border-t border-slate-100 flex flex-col gap-1">
        ...
        >
          <FiSettings size={20} className="group-hover:rotate-45 transition-transform duration-500" />
          <span className="text-sm">Configurações</span>
        </Link>

        {/* Bloco de Usuário Expandido */}
        <div className="mt-2 p-3 bg-slate-50 rounded-2xl flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-bold text-blue-600 mt-1 uppercase tracking-tighter">Premium</span>
            </div>
          </div>
          <button 
