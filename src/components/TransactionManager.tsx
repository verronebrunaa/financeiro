...
export default function TransactionManager() {
  ...
  useEffect(() => {
    ...
    const loadCategories = async () => {
      const { data } = await supabase.from("categories").select("id, name, color, parent_id");
      setCategories(data || []);
    };
    loadCategories();
  }, [loadTransactions]);
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`Tem certeza que deseja excluir ${selected.size} transação(ões)?`)) return;
    const ids = [...selected];
  ...
  const handleBulkEdit = async (fields: { category?: string; description?: string; payment_method?: string; amount?: number }) => {
    if (selected.size === 0) return;
    setBulkSaving(true);
    const updates: Record<string, unknown> = {};
    if (fields.category !== undefined) updates.category = fields.category;
    if (fields.description?.trim()) updates.description = fields.description.trim();
    if (fields.payment_method !== undefined) updates.payment_method = fields.payment_method;
    if (fields.amount !== undefined && !Number.isNaN(fields.amount)) updates.amount = fields.amount;
    ...
    return transactions
      .filter((tx) => {
        ...
        const matchesCategory =
          ...
          categories.find((c) => c.id === tx.category)?.parent_id === filterCategory;
  ...
  }, [transactions, filterMonth, filterYear, filterCategory, search, sortField, sortDirection, resolveCatName, categories]);
      ...
      {selected.size > 0 && canUse("bulk_operations") && (
        <div className="bg-blue-600 text-white rounded-2xl px-4 sm:px-6 py-3 flex items-center justify-between gap-3 animate-in slide-in-from-top-2 duration-200 shadow-lg">
          <div className="flex items-center gap-3">
            <button onClick={() => setSelected(new Set())} className="p-1.5 rounded-lg hover:bg-blue-500 transition-colors">
              <FiX size={16} />
            </button>
                  ...
                  <tr className="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em]">
                    <th className="p-5 w-12">
                      <button onClick={toggleSelectAll} className="text-slate-400 hover:text-blue-600 transition-colors">
                        <SelectAllIcon transactions={paginatedTransactions} selected={selected} />
                      </button>
                    </th>
                    <SortHeader label="Data de Competência" field="competence_date" currentField={sortField} direction={sortDirection} onSort={handleSort} />
                    <SortHeader label="Data de Vencimento" field="due_date" currentField={sortField} direction={sortDirection} onSort={handleSort} />
                    <th className="p-5">Status</th>
                    <SortHeader label="Descrição" field="description" currentField={sortField} direction={sortDirection} onSort={handleSort} />
                    <SortHeader label="Categoria" field="category" currentField={sortField} direction={sortDirection} onSort={handleSort} />
                    <SortHeader label="Valor" field="amount" currentField={sortField} direction={sortDirection} onSort={handleSort} className="text-right px-8" />
                  </tr>
                </thead>
                <tbody className="text-sm font-bold text-slate-900">
                  {paginatedTransactions.map((tx) => {
                    const statusLabel = getStatusLabel(tx);
                    const { name: catName, color: catColor } = getCategoryDisplay(tx.category);
                    ...
                    return (
                      ...
                      >
                        <td className="p-5 w-12" onClick={(e) => e.stopPropagation()}>
                          <button onClick={() => toggleSelect(String(tx.id))} className="text-slate-400 hover:text-blue-600 transition-colors">
                            {selected.has(String(tx.id)) ? <FiCheckSquare size={16} className="text-blue-600" /> : <FiSquare size={16} />}
                          </button>
                        </td>
                        ...
                        <td className="p-5 max-w-50 truncate">
                          <div className="flex items-center gap-3">
                            ...
                            {tx.recurrence_group_id && (
                              <FiRepeat className="text-slate-300 shrink-0" title="Transação Recorrente" />
                            )}
                          </div>
                        </td>
                        <td className="p-5">
                          {catColor ? (
                            <span
                              ...
                              style={{ backgroundColor: catColor, color: "#334155", display: "inline-block", minWidth: "60px", textAlign: "center" }}
                            >
                              {catName}
                            </span>
                          ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <button
                      onClick={(e) => { e.stopPropagation(); toggleSelect(String(tx.id)); }}
                      className="mt-1 text-slate-400 hover:text-blue-600 transition-colors shrink-0"
                    >
                      {selected.has(String(tx.id)) ? <FiCheckSquare size={18} className="text-blue-600" /> : <FiSquare size={18} />}
                    </button>
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${amt < 0 ? "bg-red-50 text-red-600" : "bg-emerald-50 text-emerald-600"}`}>
                        {amt < 0 ? <FiArrowDownLeft size={18} /> : <FiArrowUpRight size={18} />}
                      </div>
                      <div className="min-w-0">
                        <p className="font-bold text-slate-900 truncate text-sm">{tx.description}</p>
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                          <span className="text-[11px] text-slate-400 flex items-center gap-1">
                            <FiCalendar size={10} />
                            {formatSafeDate(tx.due_date)}
                          </span>
                          {tx.recurrence_group_id && <FiRepeat className="text-slate-300" size={10} />}
                        </div>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <p className={`font-black text-sm tracking-tight ${amt < 0 ? "text-slate-900" : "text-emerald-600"}`}>
                        {fmt(amt)}
                      </p>
                      <StatusBadge status={statusLabel} />
                    </div>
...
function BulkEditModal({
  ...
  saving: boolean;
  onClose: () => void;
  onSave: (fields: { category?: string; description?: string; payment_method?: string; amount?: number }) => void;
}>) {
  ...
  const handleSave = () => {
    const fields: { category?: string; description?: string; payment_method?: string; amount?: number } = {};
    if (editCategory) fields.category = category;
    if (editDescription && description.trim()) fields.description = description.trim();
    if (editPayment) fields.payment_method = paymentMethod;
    if (editAmount && amount) fields.amount = Number(amount);
    onSave(fields);
  };

  const hasAnyField = editCategory || editDescription || editPayment || editAmount;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 sm:p-8 animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-lg font-black text-slate-900">Edição em Massa</h3>
            <p className="text-xs text-slate-400 font-bold mt-0.5">{count} transação(ões) selecionada(s)</p>
          </div>
        ...
        <p className="text-xs text-slate-500 font-bold mb-4">Marque os campos que deseja alterar. Campos desmarcados não serão modificados.</p>

        <div className="space-y-4">
          {/* Categoria */}
          <div className="space-y-2">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={editCategory} onChange={(e) => setEditCategory(e.target.checked)} className="accent-blue-600 w-4 h-4" />
              <span className="text-xs font-black text-slate-700 uppercase tracking-wider">Categoria</span>
            </label>
            {editCategory && (
              <select
                value={category}
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={editDescription} onChange={(e) => setEditDescription(e.target.checked)} className="accent-blue-600 w-4 h-4" />
              <span className="text-xs font-black text-slate-700 uppercase tracking-wider">Descrição</span>
            </label>
            {editDescription && (
              <input
                type="text"
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={editPayment} onChange={(e) => setEditPayment(e.target.checked)} className="accent-blue-600 w-4 h-4" />
              <span className="text-xs font-black text-slate-700 uppercase tracking-wider">Método de Pagamento</span>
            </label>
            {editPayment && (
              <select
                value={paymentMethod}
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={editAmount} onChange={(e) => setEditAmount(e.target.checked)} className="accent-blue-600 w-4 h-4" />
              <span className="text-xs font-black text-slate-700 uppercase tracking-wider">Valor</span>
            </label>
            {editAmount && (
              <input
                type="number"
        ...
        <div className="flex gap-3 mt-6">
          ...
          >
            {saving ? <FiLoader className="animate-spin" size={16} /> : <FiEdit2 size={16} />}
            Aplicar
          </button>
        </div>
      </div>
...
function SelectAllIcon({ transactions, selected }: Readonly<{ transactions: Transaction[]; selected: Set<string> }>) {
  if (transactions.length > 0 && transactions.every((tx) => selected.has(String(tx.id)))) {
...
function StatusBadge({ status }: Readonly<{ status: string }>) {
  ...
  return (
    <span className={`px-2.5 py-0.5 rounded-md text-[9px] font-black tracking-widest uppercase ${cls}`}>
      {status}
    </span>
  );
}
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col text-[8px] opacity-30 group-hover/h:opacity-100 transition-opacity">
          <FiChevronUp
            className={currentField === field && direction === "asc" ? "text-blue-600 opacity-100" : ""}
          />
          <FiChevronDown
            className={currentField === field && direction === "desc" ? "text-blue-600 opacity-100" : ""}
          />
        </div>
      </div>
    </th>
